...
if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
  ...
  user.commandCount = 1
        ...
        if (!m.fromMem && m.text.match(/(ساسكي|بوت|sasuke|bot|Sasuke)/gi)) {
...
export async function callUpdate(callUpdate) {
    ...
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;MOHAMED ITACHI KUN ;;;\nFN:MOHANED ITACHI KUN \nORG:MOHAMED ITACHI KUN \nTITLE:\nitem1.TEL;waid=201114329294:+201114329294\nitem1.X-ABLabel:MOHAMED ITACHI KUN \nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME: MOHAMED ITACHI KUN \nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: 'MOHAMED ITACHI KUN ❤', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*ابتعد يا مستخدم الامر الميزه دي للمطور بس!! *',
        owner: '*ابتعد يا مستخدم الامر ده ل المطوري فقط!!*',
        mods: '*ابعد يا مستخدم الأمر الميزه دي للمطور بس!!*',
        premium: '*ابعد يا مستخدم الامر الميزه دي للناس البريم بس!!*',
        group: '*اخرس يا مستخدم الميزه دي للجروبات بس!!*',
        private: '*اخرس يا مستخدم الميزه دي للخاص بس!!*',
        admin: '*اخرس يا مستخدم الميزه دي للمشرفين بس!!*',
        botAdmin: '* هات ادمن يحب 🐱! *',
        unreg: '*[ لحظة !! انت مش مسجل ]*\n\n*『 سجل الامر عشان تفعله 』*\n*➣ #تفعيل*',
        restrict: '*『 الأمر ده المطور لغاه ! 』*'
    }[type]
    let aa = { quoted: m, userJid: conn.user.jid }
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] اشعار - تنبيه', body: 'بوت - ساسكي', thumbnail: imagen1, sourceUrl: 'http://wa.me/201114329294' }}}}, aa)
