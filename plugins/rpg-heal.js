...
const handler = async (m, {conn, args, usedPrefix, __dirname}) => {
  ...
  if (user.health >= 100) return conn.reply(m.chat, `*❤️ الصحة الخاصة بك تزيد عن 100 ❤️*\n\nالصحة: ${user.health}`, m);
...
*${usedPrefix}transfer potion ${count - user.potion}* @${conn.getName(m.sender)}`]], m)*/
  ...
  conn.reply(m.chat, `**━┈━《 ✅ تم شفاء 》━┈━*\n\nتم استخدامك ${count} من الصفقات 🥤 لشفاء الصحة الخاصة بك\𝚗الصحه » ${user.health} ❤\n\nالصحه مكتمله`, m);
};
