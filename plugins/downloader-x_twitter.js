...
if (!text) throw `*[❗] يرجى إدخال رابط من X (Twitter)، مثال: ${usedPrefix + command}* https://twitter.com/auronplay/status/1586487664274206720?s=20&t=3snvkvwGUIez5iWYQAehpw`;
if (enviando) return;
    enviando = true;
try {
 ...
 if (res?.result.type == 'video') {
     const caption = res?.result.caption ? res.result.caption : '*هنا لديك فيديو*';
    ...
    enviando = false;
    throw '*[❗] خطأ، يرجى المحاولة مرة أخرى لاحقًا.*';
    return;
  }
};    
handler.command = /^((x|xdl|dlx|twdl|tw|twt|تويتر)(dl)?)$/i;
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!id)
      return resolve({
        status: "error",
        message:
          "حدث خطأ أثناء الحصول على معرف Twitter. تأكد من صحة رابط Twitter الخاص بك!",
      });
